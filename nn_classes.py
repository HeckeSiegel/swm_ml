...
class DataGenerator_uniform:
    ...
    def __init__(self,nens,cycles,n_train,n_val,mode):
        ...
        np.savetxt('data/random_truth_'+str(nens)+'.csv',truth.reshape((1,750)),delimiter=',',fmt=['%f']*750)
        np.savetxt('data/random_background_'+str(nens)+'.csv',background,delimiter=',',fmt=['%f']*nens)
...
class DataGenerator_ml_init:
    ...
    obs_ncyc : time stamp for dynamics
    '''
    def __init__(self,ml_model,nens,samples,exp_ID,obs_ncyc):
        ...
        if ml_model=='bnn':
            ...
            # bnn makes samples*nens parameter predictions, randomly sample nens parameter set from that distribution for data assimilation
        ...
        background = sweq__(analysis,dte,n,nens,mindex,nindex,1000+dte,y_re[:,0],y_re[:,1],y_re[:,2])[:,:,-1]
...
class DataGenerator_const_init:
    ...
    obs_ncyc : time stamp for dynamics
    '''
    def __init__(self,nens,exp_ID,mode,obs_ncyc):
...
class DataGenerator_const:
    ...
    obs_ncyc : time stamp for dynamics
    '''
    def __init__(self,nens,DA_cycle,mode,obs_ncyc):
        ...
        background = sweq__(analysis_old,dte,n,nens,mindex,nindex,1000+(obs_ncyc+DA_cycle)*dte,labels_background[:,0],labels_background[:,1],labels_background[:,2])[:,:,-1]
        ...
        truth_new = sweq__(truth_old,dte,n,1,mindex,nindex,1000+(obs_ncyc+DA_cycle)*dte,[labels_truth[0]],[labels_truth[1]],[labels_truth[2]])[:,:,-1]
...
def DA(mode,DA_cycles_total,exp_ID_start,exp_ID_end,nens,samples,obs_ncyc):
    '''
    ...
    for exp_ID in range(exp_ID_start,exp_ID_end):
        if mode in ['true','false']:
            DataGenerator_const_init(nens=nens,exp_ID=exp_ID,mode=mode,obs_ncyc=obs_ncyc)
            obs_ncyc += 1
        else:
            DataGenerator_ml_init(ml_model=mode,nens=nens,samples=samples,exp_ID=exp_ID,obs_ncyc=obs_ncyc)
            obs_ncyc += 1
        rmse_spread_params_vars = np.zeros((2,6))
        for cycle in range(DA_cycles_total):
            DataGenerator_const(nens=nens,DA_cycle=cycle,mode=mode,obs_ncyc=obs_ncyc)

            params_bnn = np.loadtxt('data/online/background/labels_raw.csv',delimiter=',',skiprows=1)
            params_truth = np.loadtxt('data/online/truth/labels_raw.csv',delimiter=',',skiprows=1)

            rmse_spread_params_vars[0,:3] = np.mean((params_bnn-params_truth)**2,axis=0)**(1/2)
...
def save_parameter_preds(nens,n_train,exp_ID_total,samples,val_obs):
    ...
    for exp_ID in range(exp_ID_total):
        ...
        if val_obs:
            ...
            x = np.loadtxt('data/obs/'+str(exp_ID)+'.csv',delimiter=',')[...,H]
        else:
